...
module.exports = (
  ...
  expressApp.get(`${pathPrefix}/session`, (req, res) => {
    ...
    if (functions.session) session = functions.session(session, req)
        ...
        functions
          ...
          .then((user) => {
            ...
            providers.forEach((provider) => {
              linkedAccounts[provider.providerName] = user[
      ...
      return new Promise((resolve, reject) => {
        ...
        providers.forEach((provider) => {
          configuredProviders[provider.providerName] = {
            signin:
              (serverUrl || '') +
              `${pathPrefix}/oauth/${provider.providerName.toLowerCase()}`,
            callback:
              (serverUrl || '') +
              `${pathPrefix}/oauth/${provider.providerName.toLowerCase()}/callback`,
          }
        })
        ...
        .then((user) => {
          if (user) {
            ...
            req.logIn(user, (err) => {
              if (err)
                return res.redirect(
                  `${pathPrefix}/error?action=signin&type=credentials`,
                )
              if (req.xhr) {
                ...
                return res.redirect(
                  `${pathPrefix}/callback?action=signin&service=credentials`,
                )
              }
            })
          } else {
            // If no user object is returned, bounce back to the sign in page
            return res.redirect(`${pathPrefix}?action=emailSignin&type=invalid`)
          }
        })
        .catch((errorType) => {
          return res.redirect(
  ...
  if (functions.sendSignInEmail) {
    ...
    expressApp.post(`${pathPrefix}/email/signin`, (req, res) => {
      ...
      functions
        ...
        .catch((err) => {
          return res.redirect(
            `${pathPrefix}/error?action=signin&type=email&email=${email}`,
          )
        })
    })
    ...
    expressApp.get(`${pathPrefix}/email/signin/:token`, (req, res) => {
      if (!req.params.token) {
        return res.redirect(
          `${pathPrefix}/error?action=signin&type=token-missing`,
        )
      }

      functions
        .find({ emailToken: req.params.token })
        .then((user) => {
          if (user) {
        ...
        .then((user) => {
          ...
          req.logIn(user, (err) => {
            if (err)
              return res.redirect(
                `${pathPrefix}/error?action=signin&type=token-invalid`,
              )
            if (req.xhr) {
              // If AJAX request (from client with JS), return JSON response
              return res.json({ success: true })
              ...
              // If normal form POST (from client without JS) return redirect
              return res.redirect(
                `${pathPrefix}/callback?action=signin&service=email`,
              )
            }
          })
        })
