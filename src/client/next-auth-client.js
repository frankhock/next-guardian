...
export default class {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          return Promise.reject(
            Error('Unexpected response when trying to get CSRF token'),
          )
        }
      })
      .then((response) => response.json())
    ...
    return fetch('/auth/linked', {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          return Promise.reject(
            Error('Unexpected response when trying to get linked accounts'),
          )
        }
      })
      .then((response) => response.json())
   ...
   **/
  static async providers({ req = null } = {}) {
    // If running server side, uses server side method
  ...
  static async signin(params) {
    ...
    const route =
      typeof params === 'string' ? '/auth/email/signin' : '/auth/signin'
