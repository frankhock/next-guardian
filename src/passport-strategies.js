...
module.exports = ({
  ...
  providers.forEach(
    ({ providerName, Strategy, strategyOptions, getProfile }) => {
      ...
      passport.use(
        ...
        new Strategy(
          strategyOptions,
          (req, accessToken, refreshToken, _params, _profile, next) => {
            try {
              ...
              if (!profile.email) {
                profile.email = `${providerName.toLowerCase()}-${
                  profile.id
              ...
              if (autoLinkAccounts) {
                findParams = Object.assign(findParams, { email: profile.email })
              }
                      ...
                      return functions
                        ...
                        .then((user) => {
                          ...
                          if (!user)
                            return next(
                              new Error(
                                'Unable to look up account for current user',
                              ),
                              false,
                            )
  ...
  providers.forEach(({ providerName, providerOptions }) => {
    ...
    expressApp.post(
      `${pathPrefix}/oauth/${providerName.toLowerCase()}/unlink`,
      (req, res, next) => {
        if (!req.user) {
        ...
        functions
          ...
          .then((user) => {
            if (!user)
              return next(
                new Error('Unable to look up account for current user'),
              )
            ...
            return functions
              .update(user, null, { delete: providerName.toLowerCase() })
              .then((user) => {
                return res.redirect(
                  `${pathPrefix}/callback?action=unlink&service=${providerName.toLowerCase()}`,
                )
              })
              .catch((err) => {
                return next(err, false)
